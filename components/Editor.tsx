...
const Editor: React.FC = () => {
  ...
  const handlePaste = (e: React.ClipboardEvent<HTMLTextAreaElement>) => {
    // If no clipboard data (common on some mobile browsers), let default behavior happen
    if (!e.clipboardData) {
      return;
    }
      ...
      <div className="relative">
        <textarea
          ...
          onChange={(e) => {
            ...
            // Gboard fix: Detect large insertion (> 5 chars) that didn't trigger onPaste
            if (diff > 5) {
              // Check if the new text contains markdown symbols
              if (newValue.includes('**') || newValue.includes('*') || newValue.includes('__') || newValue.includes('`') || newValue.includes('~~')) {
                // We could try to strictly target the inserted text.
